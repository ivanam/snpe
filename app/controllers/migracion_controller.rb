  ...
  load_and_authorize_resource :class => false
	...
	def migrar_hs
		...
		if nueva
		    ...
		    res= client.query("SELECT secuencia as secMax, p.* FROM his_padhc p where  p.escuela= '"+e.to_s+"' and mes = 7 and anio = 2018  and secuencia<88 and (estado = 'ALT' or estado='LIC') and fecha_baja='0000-00-00'")
		end
	...
	def migrar_cargos
	    ...
	    @escuelas.each do |e|
	    	...
	    	if nueva
		    	...
		    	res= client.query("SELECT secuencia as secMax, p.* FROM his_paddoc p where  p.escuela= '"+e.to_s+"'and secuencia<88 and mes = 7 and anio = 2018 and (estado = 'ALT' or estado='LIC') and fecha_baja='0000-00-00' ")
		    end
		...
		@escuelas.each do |e|
			if nueva
	 		...
	 		select secuencia as secMax, p.* from padaux p where escuela = '"+e.to_s+"' and secuencia<88 and estado= 'LIC'  group by  nume_docu, secuencia, planta_pre, tipo_emp, cargo_r")
				...
				res= client.query("SELECT secuencia as secMax, p.* FROM his_padaux p where  p.escuela= '"+e.to_s+"'and secuencia<88 and mes = 7 and anio = 2018 and (estado = 'ALT' or estado='LIC') and fecha_baja='0000-00-00'")
	 	    end
