...
def cancelar_sin_goce
  ...
  if !lic.update(:cancelada_sin_goce => 1, obs_sin_goce_cancelacion: params["observaciones"], user_cheq_finalizada_id: current_user.id)
...
#-----------------------------------------------------------licencias sin goce -----------------------------------------
  ...
  def guardar_licencia_horas
    ...
    con_certificado = params['con_certificado'].to_i
     ...
     @oficinaOr = Establecimiento.where(id: altbahora.establecimiento_id).first
     nro_oficinaOr = @oficinaOr.codigo_jurisdiccional
    ...
    if Articulo.find(params['articulo']).permite_otro_organismo and params[:permite_otro_organismo].empty?
      ...
      if listaArtTrasladosTrans.include? params[:articulo] and altbahora.secuencia != 1000
          if params[:destino].to_i > 0
            @oficinaDes = Establecimiento.where(id: params[:destino]).first
  ...
  def guardar_licencia_cargos
    ...
    nro_documento = @persona.nro_documento
    userLic = current_user.id
    @oficinaOr = Establecimiento.where(id: cargo.establecimiento_id).first
    nro_oficinaOr = @oficinaOr.codigo_jurisdiccional
    ...
    if Articulo.find(params['articulo']).permite_otro_organismo and params[:permite_otro_organismo].empty?
      msg = "Complete el campo Organismo"
      render json: msg.to_json 
    elsif licencias_anuales.include? art_id and params[:fecha_anio_lic_1] == nil
      msg = "Complete el año de la licencia"
      render json: msg.to_json 
      ...
      if listaArtTrasladosTrans.include? params[:articulo] and cargo.secuencia != 1000
          ...
          @licencia = Licencium.new(con_certificado: con_certificado, con_formulario: con_formulario, cargo_id: params[:id_cargos], fecha_desde: params[:fecha_inicio], fecha_hasta: params[:fecha_fin], articulo_id: params[:articulo], organismo: params[:permite_otro_organismo], vigente: "Vigente", destino: @oficina.id, observaciones: params[:observaciones], nro_documento: nro_documento, user_id: userLic, oficina: nro_oficinaOr)
  ...
  def guardar_licencia_cargos_no_docentes
    ...
    nro_documento = persona.nro_documento
    userLic = current_user.id
    @oficinaOr = Establecimiento.where(id: cargonds.establecimiento_id).first
    nro_oficinaOr = @oficinaOr.codigo_jurisdiccional
    ...
    if Articulo.find(params['articulo']).permite_otro_organismo and params[:permite_otro_organismo].empty?
      msg = "Complete el campo Organismo"
      render json: msg.to_json 
    elsif licencias_anuales.include? art_id and params[:fecha_anio_lic_2] == nil
      msg = "Complete el año de la licencia"
      render json: msg.to_json 
      ...
      # ----------------traslados definitivo
        ...
        if  CargoNoDocente.find(params['id_cargos_no_docentes']).update(establecimiento_id: @oficina.id, estado: 'REU')
            Traslado.create!(:cargo_no_docente_id => params['id_cargos_no_docentes'], user_id: current_user.id, fecha_cambio_oficina: params[:fecha_inicio], disposicion: params[:disposicion])
           render json: 0
        else
  ...
  def guardar_licencia_cargos_no_docentes2
       ...
       #BUSCO LA LICENCIA ANTERIOR Y VERIFICO SI TIENE FECHA HASTA
        if @licencia_anterior.fecha_hasta == nil
            if params[:fecha_inicio].to_date > @licencia_anterior.fecha_desde.to_date
              ...
              @licencia = Licencium.new(con_certificado: con_certificado, con_formulario: con_formulario, cargo_no_docente_id: params[:id_cargos_no_docentes], fecha_desde: params[:fecha_inicio], fecha_hasta: params[:fecha_fin], articulo_id: params[:articulo], organismo: params[:permite_otro_organismo], vigente: "Vigente", destino: @oficina.id, observaciones: params[:observaciones], nro_documento: nro_documento, user_id: userLic, oficina: nro_oficinaOr)
    ...
    #---------------LICENCIAS COMUNES 
        ...
        if listaArtTrasladosTrans.include? @licencia_anterior.articulo_id
          ...
          cargo1000 = CargoNoDocente.where(:persona_id => cargo.persona_id, :secuencia => 1000, obs_lic: descripcion_articulo2 ).where.not(estado: "BAJ").first
          cargo1000.update!(:estado => "BAJ") 
          @licencia = Licencium.new(cargo_id: params[:id_cargos_no_docentes], fecha_desde: params[:fecha_inicio], fecha_hasta: params[:fecha_fin], articulo_id: params[:articulo], organismo: params[:permite_otro_organismo], vigente: "Vigente", anio_lic: params[:fecha_anio_lic_7], observaciones: params[:observaciones], nro_documento: nro_documento, user_id: userLic, oficina: nro_oficinaOr)
          @licencia_anterior.update!(vigente: 'Finalizada', fecha_hasta:  fecha, por_continua: 1, prestador_id: prestador_3 )                                                                                                                                      
        end 
        if @licencia_anterior.fecha_hasta == nil
            if params[:fecha_inicio].to_date > @licencia_anterior.fecha_desde.to_date
  ...
  private
    ...
    def licencium_params
      params.require(:licencium).permit(:altas_bajas_hora_id, :fecha_desde, :fecha_hasta, :articulo_id, :cargo_id, :cargo_no_docente_id, :vigente, :prestador_id, :observaciones, :anio_lic, :destino, :disposicion)
    end
end
