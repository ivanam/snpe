class AltasBajasHora < ActiveRecord::Base
  ...
  if true
    ...
    validates :plan_id, :presence => true, if: :no_es_licencia_para_baja, if: :motivo_baja_con_controles
    validates :materium_id, :presence => true, if: :no_es_licencia_para_baja, if: :motivo_baja_con_controles
    validates :turno, :presence => true, if: :no_es_licencia_para_baja, if: :motivo_baja_con_controles


  # # #Validación de alta
    before_create :validar_horas
    validate :validar_alta, if: :motivo_baja_con_controles
  ...
  PLANES_SIN_VALIDACION = [3000,261,256] #Listado de planes que no requieren validacion
  LONGITUD_CODIGO = 4
  ...
  def validar_reemplazante
    if self.situacion_revista == '1-3' 
      ...
      if alta_horas.estado != "LIC"
        errors.add(:base,"Las horas a reemplazar no se encuentra con licencia sin goce de haberes.")                
      end
      if alta_horas.estado == "ALT"
        errors.add(:base,"En las horas a reemplazar ya se encuentra una persona de alta")   
      end
  ...
  def validar_suplente
    if self.situacion_revista == '2-3' || self.situacion_revista == '2-4' 
      ...
      if alta_horas.estado == "ALT"
        errors.add(:base,"En las horas a suplantar ya se encuentra una persona de alta")   
      end
