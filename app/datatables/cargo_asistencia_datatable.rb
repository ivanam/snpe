  ...
  def data
    records.map do |record|
      [
        ...
        '<span class="lleg_tarde_justificada" data-type="text" data-resource="post" data-name="lleg_tarde_justificada" data-url="'+Rails.application.routes.url_helpers.asistencia_editar_asistencia_cargo_path(record.id.to_s, anio: options[:anio], mes: options[:mes])+'">'+record.lleg_tarde_justificada(options[:anio], options[:mes])+'</span>',
        '<span class="lleg_tarde_injustificada" data-type="text" data-resource="post" data-name="lleg_tarde_injustificada" data-url="'+Rails.application.routes.url_helpers.asistencia_editar_asistencia_cargo_path(record.id.to_s, anio: options[:anio], mes: options[:mes])+'">'+record.lleg_tarde_injustificada(options[:anio], options[:mes])+'</span>',
      ]
    end
  end
