  ...
  def data
    records.map do |record|
      [
        ...
        '<span class="observaciones" data-type="text" data-resource="post" data-name="observaciones" data-url="'+Rails.application.routes.url_helpers.asistencia_editar_asistencia_cargo_no_docente_path(record.id.to_s, anio: options[:anio], mes: options[:mes])+'">'+record.observaciones_por_periodo(options[:anio], options[:mes])+'</span>',
      ]
    end
  end
