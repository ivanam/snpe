class AltasBajasHoraBajaIngresadasDatatable < AjaxDatatablesRails::Base
    ...
    records.map do |record|
      [
        ...
        if record.estado_actual == "Ingresado_Baja"  then
          ...
          '<button class="btn btn-'+record.estados.last.color_estado+' btn-xs pepe" data-toggle="modal" data-target="#modal_bajas_horas_efectivas" alta-id="'+record.id.to_s+'"><b>'+record.estados.last.mensaje_estado+'</b></button>'
        end,
      ]
    end
  end
