 ...
 #---------------------------------licencias sin goce ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	...
	def listado_de_licencias_cargonds_sg(mindate2, maxdate2, dni2, art2)
		if current_user.role? :personal or current_user.role? :sadmin  or current_user.role? :licencia then
            ...
            if dni2 == nil and art2 != nil
            @licenciasCndsSg = Licencium.select('licencia.*').from('licencia, cargo_no_docentes, establecimientos, personas, ').where('licencia.cargo_no_docente_id = cargo_no_docentes.id AND cargo_no_docentes.establecimiento_id = establecimientos.id AND cargo_no_docentes.persona_id = personas.id').where('fecha_desde >= ?', mindate2).where('articulo_id = ?', art2).where('vigente not in ("Cancelada")')
		    end
		    if  dni2 != nil and art2 != nil
            @licenciasCndsSg = Licencium.select('licencia.*').from('licencia, cargo_no_docentes, establecimientos, personas').where('licencia.cargo_no_docente_id = cargo_no_docentes.id AND cargo_no_docentes.establecimiento_id = establecimientos.id AND cargo_no_docentes.persona_id = personas.id').where('fecha_desde >= ?', mindate2).where('articulo_id = ?', art2).where('nro_documento= ?', dni2).where('vigente not in ("Cancelada")')
		    end
		else
			if  dni2 == nil and  art2 == nil	
			@licenciasCndsSg = Licencium.select('licencia.*').from('licencia, cargo_no_docentes, establecimientos, personas').where('licencia.cargo_no_docente_id = cargo_no_docentes.id AND cargo_no_docentes.establecimiento_id = establecimientos.id AND cargo_no_docentes.persona_id = personas.id').where(cargo_no_docentes: {establecimiento_id: session[:establecimiento]}).where('fecha_desde >= ?', mindate2).where('con_goce = 0').where('vigente not in ("Cancelada")')
			end
			if dni2 != nil and  art2 == nil
