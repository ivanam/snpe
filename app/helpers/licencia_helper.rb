module LicenciaHelper
	...
	def listado_de_licencias(mindate, maxdate)	
		...
		if current_user.role? :personal or current_user.role? :sadmin then  
		    @licencias = Licencium.select('establecimientos.*, personas.*, licencia.*, altas_bajas_horas.*, articulos.*, licencia.observaciones').from('licencia, altas_bajas_horas, establecimientos, personas, articulos').where('licencia.altas_bajas_hora_id = altas_bajas_horas.id AND altas_bajas_horas.establecimiento_id = establecimientos.id AND altas_bajas_horas.persona_id = personas.id AND licencia.articulo_id = articulos.id').where('fecha_desde >= ?', mindate).where('articulo_id in (242,243,244,245,291,292,293,294,263,264,265,266,299,300,321,322,323,324,325)')
		elsif current_user.role? :licencia 		
			@licencias = Licencium.select('establecimientos.*, personas.*, licencia.*, altas_bajas_horas.*, articulos.* , licencia.observaciones').from('licencia, altas_bajas_horas, establecimientos, personas , articulos').where('licencia.altas_bajas_hora_id = altas_bajas_horas.id AND altas_bajas_horas.establecimiento_id = establecimientos.id AND altas_bajas_horas.persona_id = personas.id AND licencia.articulo_id = articulos.id').where('fecha_desde >= ?', mindate).where('articulo_id in (242,243,244,245,291,292,293,294,263,264,265,266,299,300,321,322,323,324,325)').where('establecimientos.codigo_jurisdiccional in ' + Establecimiento.estab_sedes)
		else			
			@licencias = Licencium.select('establecimientos.*, personas.*, licencia.*, altas_bajas_horas.*, articulos.* , licencia.observaciones').from('licencia, altas_bajas_horas, establecimientos, personas, articulos').where('licencia.altas_bajas_hora_id = altas_bajas_horas.id AND altas_bajas_horas.establecimiento_id = establecimientos.id AND altas_bajas_horas.persona_id = personas.id AND licencia.articulo_id = articulos.id').where(altas_bajas_horas: {establecimiento_id: session[:establecimiento]}).where('fecha_desde >= ?', mindate).where('articulo_id in (242,243,244,245,291,292,293,294,263,264,265,266,299,300,321,322,323,324,325)')
		end	
	...
	def listado_de_licencias_cargonds(mindate2, maxdate2)
		if current_user.role? :personal or current_user.role? :sadmin then			
			@licenciasCnds = Licencium.select('establecimientos.*, personas.*, licencia.*, cargo_no_docentes.*, articulos.* , licencia.observaciones').from('licencia, cargo_no_docentes, establecimientos, personas, articulos').where('licencia.cargo_no_docente_id = cargo_no_docentes.id AND cargo_no_docentes.establecimiento_id = establecimientos.id AND cargo_no_docentes.persona_id = personas.id AND licencia.articulo_id = articulos.id').where('fecha_desde >= ?', mindate2).where('articulo_id in (242,243,244,245,291,292,293,294,263,264,265,266,299,300,321,322,323,324,325)')
		elsif current_user.role? :licencia
		   @licenciasCnds = Licencium.select('establecimientos.*, personas.*, licencia.*, cargo_no_docentes.*, articulos.*, licencia.observaciones').from('licencia, cargo_no_docentes, establecimientos, personas, articulos').where('licencia.cargo_no_docente_id = cargo_no_docentes.id AND cargo_no_docentes.establecimiento_id = establecimientos.id AND cargo_no_docentes.persona_id = personas.id AND licencia.articulo_id = articulos.id').where('fecha_desde >= ?', mindate2).where('articulo_id in (242,243,244,245,291,292,293,294,263,264,265,266,299,300,321,322,323,324,325)').where('establecimientos.codigo_jurisdiccional in ' + Establecimiento.estab_sedes) 
		else	
