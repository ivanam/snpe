module LicenciaHelper
	...
	def listado_de_licencias_cargonds(mindate2, maxdate2)
		if current_user.role? :personal or current_user.role? :sadmin then			
			@licenciasCnds = Licencium.select('establecimientos.*, personas.*, licencia.*, cargo_no_docentes.*, articulos.* , licencia.observaciones').from('licencia, cargo_no_docentes, establecimientos, personas, articulos').where('licencia.cargo_no_docente_id = cargo_no_docentes.id AND cargo_no_docentes.establecimiento_id = establecimientos.id AND cargo_no_docentes.persona_id = personas.id AND licencia.articulo_id = articulos.id').where('fecha_creacion = ?', maxdate2).where('articulo_id in (242,243,244,245,291,292,293,294,263,264,265,266,299,300,321,322,323,324,325)')
		elsif current_user.role? :licencia
	...
	def listado_parte_diario(mindate, maxdate)	
		...
		if current_user.role? :personal or current_user.role? :sadmin then  
		    ...
		    @liceCN = Licencium.select('establecimientos.*, personas.*, licencia.*, cargo_no_docentes.*, articulos.* , licencia.observaciones').from('licencia, cargo_no_docentes, establecimientos, personas, articulos').where('licencia.cargo_no_docente_id = cargo_no_docentes.id AND cargo_no_docentes.establecimiento_id = establecimientos.id AND cargo_no_docentes.persona_id = personas.id AND licencia.articulo_id = articulos.id').where('fecha_creacion = ?', mindate).where('articulo_id in (242,243,244,245,291,292,293,294,263,264,265,266,299,300,321,322,323,324,325)')
		    @licenciasC = Licencium.select('establecimientos.*, personas.*, licencia.*, cargos.*, articulos.*, licencia.observaciones').from('licencia, cargos, establecimientos, personas, articulos').where('licencia.cargo_id = cargos.id AND cargos.establecimiento_id = establecimientos.id AND cargos.persona_id = personas.id  AND licencia.articulo_id = articulos.id').where('fecha_creacion = ?', mindate).where('articulo_id in (242,243,244,245,291,292,293,294,263,264,265,266,299,300,321,322,323,324,325)')
