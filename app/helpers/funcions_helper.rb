module FuncionsHelper
  ...
  def funciones_permitidas
    if Establecimiento.find(session[:establecimiento].to_i).nivel_id.to_i == 1 
      return Funcion.where(categoria: [802, 803, 111, 511, 809, 811, 112, 117, 312, 212, 812, 319, 817, 818, 819, 808] )
    elsif Establecimiento.find(session[:establecimiento].to_i).nivel_id.to_i == 2 
    ...
    elsif Establecimiento.find(session[:establecimiento].to_i).nivel_id.to_i == 3
    	return Funcion.all
    else
      return []
    end
